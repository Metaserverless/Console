({
  ...
  async method() {
    ...
    const schemas = files.filter((file) => file.indexOf('.') !== 0 && file.slice(-3) === '.js');

    dirPath = './application/flow/';
    files = await node.fsp.readdir(dirPath);
    const flows = files.filter((file) => file.indexOf('.') !== 0 && file.slice(-3) === '.md');

    dirPath = './application/api/' + tenant + '.1/';
    files = await node.fsp.readdir(dirPath);
    const apis = files.filter((file) => file.indexOf('.') !== 0 && file.slice(-3) === '.js');
